...
def main() -> None:
    # Settings
    samplesize = int(10)
    subject = int(3) # subject % 3 = 0
    foldersize = int(samplesize*subject/3)
    source = "lfw" # lfw, CelebA-HQ
    target = "CelebA-HQ" # lfw, CelebA-HQ
    dfr_model = 'facenet' # facenet, insightface
    threshold = 0.7032619898135847 # similarity score
    attack_model = attacks.LinfPGD
    loss_type = 'MT' #'ST', 'MT', 'MS', 'C-BCE'
    epsilons = 0.03
    steps = 1000
    step_size = 0.001
    ...
    target_index = list(range(foldersize,foldersize*2))+list(range(foldersize*2,foldersize*3))+list(range(0,foldersize))
    target_features = features[target_index]
    target_index2 = list(range(foldersize*2,foldersize*3))+list(range(0,foldersize))+list(range(foldersize,foldersize*2))
    target_features2 = features[target_index2]
    del source_images
    ...
    for i in range(ceil(totalsize/batchsize)):
        ...
        raw_advs_tmp, _, _ = attack(fmodel, attack_images[start:start+batchsize], target_features[start:start+batchsize], criterion2 = target_features2[start:start+batchsize], epsilons=epsilons)
        end_time = time.time()
    ...
    # Save advs
    save_image(raw_advs[0], f'results/images/{loss_type}_{source}_{target}_{dfr_model}_{log_time}_adv.jpg')
    ...
    similarity1 = cos_similarity_score(advs_features,target_features)
    similarity2 = cos_similarity_score(advs_features,target_features2)
    similarity = torch.cat((similarity1,similarity2),0)
    dissimilarity = 1-(similarity).mean()
    print(f"Dissimilarity = {dissimilarity}")
    f.write(f"Dissimilarity = {dissimilarity}\n")
    
    # Compute FMR
    fmr_target1, fmr_renew1 = FMR(advs_features, target_features, threshold, samplesize)
    fmr_target2, fmr_renew2 = FMR(advs_features, target_features2, threshold, samplesize)
    fmr_target = (fmr_target1+fmr_target2)/2
    fmr_renew = (fmr_renew1+fmr_renew2)/2
