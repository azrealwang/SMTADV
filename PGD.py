...
def main() -> None:
    ...
    threshold = 0.5854403972629942  # Similarity
    attack_model = attacks.LinfPGD
    loss_type = 'C-BCE'  # 'ST', 'MT', 'MS', 'C-BCE'
    epsilons = 0.03
    steps = 40
    step_size = 0.001
    ...
    f = codecs.open(f'results/logs/{loss_type}_{source}_{target}_{dfr_model}_{attack_model.__module__}_{log_time}.txt',
                    'a', 'utf-8')
    ...
    source_images, _ = ep.astensors(
        *samples(fmodel, dataset=f"{source}_test", batchsize=subject * samplesize, model_type=dfr_model))

    # Input data
    attack_index = list(range(samplesize * subject))
    attack_images = source_images[attack_index]
    target_index = list(range(foldersize, foldersize * 2)) + list(range(samplesize, foldersize)) + list(
        range(0, samplesize))
    target_features = features[target_index]
    del source_images

    # Run attack
    attack = attack_model(threshold=threshold, loss_type=loss_type, steps=steps, abs_stepsize=step_size,
                          convergence_threshold=convergence_threshold, device=f'{device}')
    ...
    for i in range(ceil(totalsize / batchsize)):
        ...
        raw_advs_tmp, _, _ = attack(fmodel, attack_images[start:start + batchsize],
                                    target_features[start:start + batchsize], epsilons=epsilons)
        end_time = time.time()
    ...
    savemat(f'mat/{loss_type}_{source}_{target}_{dfr_model}_templates.mat',
            mdict={f"{loss_type}_{source}_{target}_{dfr_model}_templates": adv_template})
